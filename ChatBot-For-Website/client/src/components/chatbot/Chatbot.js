...
class Chatbot extends Component {
    ...
    async df_client_call(request) {
        ...
        try {
            ...
            const res = await axios.post(
                'https://dialogflow.googleapis.com/v2/projects/' + 'msit-vpjtca' +
            ...
            // setTimeout(function(){that.setState({clientToken: 'LALALA'})
    ...
    async componentDidMount() {
        ...
        if (window.location.pathname === '/shop' && !this.state.cousrseWelcomeSent) {
            await this.resolveAfterXSeconds(1);
            this.df_event_query('WELCOME_MSIT');
            this.setState({ cousrseWelcomeSent: true });

        }

        this.props.history.listen(() => {
            console.log('listening');
            if (this.props.history.location.pathname === '/shop' && !this.state.cousrseWelcomeSent) {
                this.df_event_query('WELCOME_MSIT');
                this.setState({ cousrseWelcomeSent: true });
    ...
    _handleQuickReplyPayload(event, payload, text) {
        ...
        switch (payload) {
            case 'recommendations_yes':
                this.df_event_query.apply('SHOW_RECOMMENDATIONS');
                break;
    ...
    render() {
        if (this.state.showBot) {
            return (
                <div style={{ minHeight: 500, maxHeight: 300, width:400, position: 'absolute', bottom: 0, right: 0, border: '1px solid lightgray'}}>
                    <nav>
